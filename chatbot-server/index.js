...
// System prompt - kept separate and sent with every request
const SYSTEM_PROMPT = `Bạn là trợ giảng chuyên về Tư tưởng Hồ Chí Minh và chủ đề Độc lập dân tộc, Chủ nghĩa xã hội. 
...
// In-memory session store - now only stores conversation history
...
app.post("/chat", async (req, res) => {
    try {
        ...
        // Initialize session if it doesn't exist (empty array now)
        if (!sessions.has(sid)) {
            sessions.set(sid, []);
        }
        ...
        // Keep only last 8 messages in history (4 exchanges)
        const maxHistory = 8;
        const recentHistory = history.slice(-maxHistory);

        // Build the contents array: system prompt + conversation history
        const contents = [
            {
                role: "user",
            ...
            {
                role: "model",
                parts: [{ text: "Tôi hiểu. Tôi sẽ trả lời các câu hỏi dựa trên nội dung bài học về Tư tưởng Hồ Chí Minh." }]
            },
            ...recentHistory
        ];

        console.log("Sending to Gemini with", contents.length, "messages");
        ...
        // Add bot reply to history (but NOT the system prompt)
